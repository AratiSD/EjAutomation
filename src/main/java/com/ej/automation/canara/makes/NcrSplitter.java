...
public class NcrSplitter {
	...
	public void ncrSplitter_bk(String currentFile, String bankId, String terminalId, String maker, String sourcePath,
			String destinationPath, String nonSplitPath, String uploadFolderPath) {
	...
	public void ncrSplitter(String filename,String bankId,String terminalId,String maker, String sourcePath,
			String destinationPath, String nonSplitPath, String uploadFolderPath)
	{
		...
		try {
			...
			File outputFilename = null;
			BufferedWriter bufferedWriter = null;
			bigFile = new BigFile(filename);
			Iterator<String> iterString = bigFile.iterator();
			while (iterString.hasNext())
			{
				String string = iterString.next();
				if (string.contains("[0r(1)2[000p[040qe1w3h162")
						&& fileExist != null)
				{
					String st1=string.replace("", "");
					String st2=st1.replace("","");
					string = st2.replace(
							"[0r(1)2[000p[040qe1w3h162",
							"****************************************\n");
					if (string.contains("TERM") && string.contains("DATE") && string.contains("TIME"))
					{
						transactionCheck= transactionCheck+1;
						readLineByLine.append(System.lineSeparator());
						readLineByLine.append("****************************************");
						readLineByLine.append(System.lineSeparator());
						...
						date = recordSplit[0];
						transactionDate = CommonUtil.getDate(recordSplit[0], "dd/MM/yy",0);
						transactionDateSplit = transactionDate.split("-");
						fileExist = transactionDates.contains(recordSplit[0]);
						if (!fileExist) 
						{
					...
					if (!fileExist)
					{
						outputFilename = CommonUtil.folderCreation(sourcePath
								+ "/" + bankId + "/" + transactionDateSplit[2]
								+ "/" + transactionDateSplit[1] + "/"
								+ transactionDateSplit[0], date, terminalId,
								bankId, maker);
						/*outputFilename = folderCreation(SplitFile.filePath
								+ "/" + bankId + "/" + transactionDateSplit[0]
								+ "/" + transactionDateSplit[1].substring(0, 3).toUpperCase() + " "+dateParser(transactionDateSplit[1])
								+ "/" + transactionDateSplit[2], date, terminalId,
								bankId, maker);*/
				...
				else if (string.matches("##STAN##[0-9]{12}") && transactionDateSplit != null)
				{
					readLineByLine.append(System.lineSeparator());
					readLineByLine
							.append("****************************************");
				...
				else if (string.contains("TERM") && string.contains("DATE") && string.contains("TIME"))
				{
					transactionCheck = transactionCheck+1;
					readLineByLine.append(System.lineSeparator());
					readLineByLine.append("****************************************");
				...
				{
					if(string.contains("WITHDRAWAL TRANSACTION") && string.contains("CARD NUMBER"))
					{
						String[] st=string.split("                                                       ");
						for(int i=0;i<st.length;i++)
						{
							st[i]=st[i].trim();
							readLineByLine.append(st[i]);
							readLineByLine.append(System.lineSeparator());
						}
					}
				...
				if (readLineByLine.toString().length() > 0
						&& transactionDates.size() > 0)
				{
					outputFilename = CommonUtil.folderCreation(sourcePath + "/"
							+ bankId + "/" + transactionDateSplit[2] + "/"
							+ transactionDateSplit[1] + "/"
							+ transactionDateSplit[0], date, terminalId, bankId,
							maker);
					/*outputFilename = folderCreation(SplitFile.filePath
							+ "/" + bankId + "/" + transactionDateSplit[0]
							+ "/" + transactionDateSplit[1].substring(0, 3).toUpperCase() + " "+dateParser(transactionDateSplit[1])
			...
			/*if(transactionCheck==0)
			{
				...
				moveFile(filename, SplitFile.notSplittedFilePath+ "\\" + file.getName());
			}*/
			if(transactionCheck>0)
			{
				bigFile.Close();
				/*
				 * File file = new File(filename); if(file.isFile()) file.delete();
				 */
			}
		...
		{
			File file = new File(filename);
			CommonUtil.moveFile(filename, nonSplitPath+ "\\" + file.getName());
			/*File file2=new File(SplitFile.notSplittedFilePath+ "\\" + file.getName());
			moveFile1(bigFile, file2,readLineByLine);*/
			bigFile.Close();
			e.printStackTrace();
			/*bigFile.Close();
			File file = new File(filename);
			boolean flag = moveFile(filename, SplitFile.notSplittedFilePath
					+ "\\" + file.getName());
			System.out.println("File moved" + flag);*/
		} 
		finally 
		{
			bigFile.Close();
